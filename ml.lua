-----------------
-- Morelight - enhanced Lua utilites module based on Microlight
--
-- Wang HongQin, 2016; License MIT
-- @module ml
...
local collectuntil = ml.collectuntil
...
function ml.collectn (n,...)
    return collectuntil(function(_,i) return i > n end,...)
end
...
-- @param f a function accepts single argument.
-- @param t the array
-- @param ... any extra arguments to the function
-- @return a new array with elements `f(t[i],...)`
-- CHANGE: exchanged args positions && make f only accepts single argument.
function ml.imap(t,f)
    return mapextend({},1,false,f,t)
end

--- map a function over a table.
-- @param f(v,k) returns a new v
-- @param t the array
-- @return a new table, {[k] = f(t[k],k)}
-- NEW
function ml.vkmap(t,f)
    local res = {}
...
--- apply a function over a table.
-- @param f a function of one argument
-- @param t the table
-- @return the table `t` iteself
-- NEW
function ml.vkapply(t,f)
    for k,v in pairs(t) do
        t[k] = f(v,k)
    end
    return t
...
-- CHANGE_NAME iapply <- transform
function ml.iapply (f,t,...)
...
-- CHANGE: pred accepts only one param.
...
-- fold an array.
-- @param l an array
-- @param start origin value to be accumulated
-- @param f(start,v) return a new accum value
-- @usage ifold({1,2,3},10,function(s,v) return s+v end) -> 16
-- NEW
function ml.ifold(l, start, f)
    for i = 1,#l do
        start = f(start,l[i])
    end
    return start
...
-- @param t a table
-- @param start origin value to be accumulated.
-- @param f(accum,v,k) return a new accum value
-- @usage vkfold({foo=3,bar=4},10,function(s,v) return s+v end) -> 17
-- NEW
function ml.vkfold(t, start, f)
    for k,v in pairs(t) do
        start = f(start,v,k)
    end
    return start
...
--- copy fileds from a table to another table
-- @param nt table to be copied to
-- @param t table to be copied from
-- @param ... fields
-- @return `nt` itself.
-- NEW
function ml.choose(nt,t, ...)
    for i =1,select("#",...) do
        local k = select(i,...)
        nt[k] = t[k]
    end
...
--- extract the values of a table as an array
-- @param t a table
-- @return an array of values
function ml.values(t)
    local i = 1
    local res = {}
...
--- The random function `ml` used.
-- default: math.random
ml.random = math.random

--- random a item from an array
-- @param l The input array
-- @return the item and its index
function ml.choice(l)
    if #l == 0 then
        return
    end
    local idx = ml.random(#l) 
    return l[idx],idx
end

--- weighted choice from an array
-- @param l The input array
-- @param _w the function returns an item's weight(must be an non-negative integer), default: item.weight
-- @return the item and its index. 
function ml.wtchoice(l, _w)
    _w = _w or function (v) return v.weight end
    local total = 0
...
function ml.shuffle(l)
    for i = #l,2,-1 do
        local r = ml.random(1,i)
        local tmp = l[i]
        l[i] = l[r]
        l[r] = tmp
    end
